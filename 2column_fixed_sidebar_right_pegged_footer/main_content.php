
<?php include( "../nbi_css_includes/css_layouts_preamble.html" ); ?>

                        <hr />

                        <h3>2 Column: Fixed Width with Right Side Bar, Content Area, and Footer that is Fixed to the Bottom of the Page</h3>

                        <p>A straight-forward CSS layout.  Because of the fixed widths this one doesn't seem to have problems with any modern browsers.</p>

                        <p>See the CSS markup for more details on this layout.</p>

								<ul>
									<li><a href="main_content.html">With content in only the center column</a></li>
									<li><a href="nav_bar.html">With content only in the side bar</a></li>
									<li><a href="none.html">With no content</a></li>
								</ul>

                        <hr />

<?php include( "../nbi_css_includes/css_layouts_overview.html" ); ?>

							<ul>
								<li><strong>Windows XP, 7</strong>
									<ul>
										<li>Internet Explorer 7 (IE 7)</li>
										<li>Internet Explorer 8 (IE 8)</li>
										<li>Internet Explorer 9 (IE 8)</li>
										<li>Internet Explorer 10 (IE 10)</li>
										<li>Internet Explorer 11 (IE 11)</li>
										<li>Chrome 33.0</li>
										<li>Firefox 12, 28 or greater</li>
									</ul>
								</li>
								<li><strong>Mac OS X</strong>
									<ul>
										<li>Safari 1.2.3</li>
									</ul>

								<li><strong>Linux</strong>
									<ul>
										<li>Firefox 27.x or greater</li>
									</ul>
								</li>
							</ul>	

                        <hr />

<?php include( "../nbi_css_includes/css_layouts_license.html" ); ?>

                        <hr />

<?php include( "../nbi_css_includes/css_layouts_w3c_buttons.html" ); ?>
